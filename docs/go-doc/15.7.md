...
如果只是想简单地把 `Execute()` 方法的第二个参数用于替换，使用 <code v-pre>{{.}}</code>。

当在浏览器环境中进行这些步骤，应首先使用 `html` 过滤器来过滤内容，例如 <code v-pre>{{html .}}</code>， 或者对 `FieldName` 过滤：<code v-pre>{{ .FieldName |html }}</code>。
...
运行 `Execute` 产生的结果来自模板的输出，它包含静态文本，以及被 <code v-pre>{{}}</code> 包裹的称之为*管道*的文本。例如，运行这段代码（示例 15.15 [pipline1.go](examples/chapter_15/pipeline1.go)）：
```go
...
点号（`.`）可以在 Go 模板中使用：其值 <code v-pre>{{.}}</code> 被设置为当前管道的值。
...
`range-end` 结构格式为：<code v-pre>{{range pipeline}} T1 {{else}} T0 {{end}}</code>。
...
预定义函数也在 [15.6节](15.6.md) 中使用：<code v-pre>{{ printf "%s" .Body|html}}</code>，否则字节切片 `Body` 会作为数字序列打印出来。
